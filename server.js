...
app.post('/api/glp/status/generate-group', requirePI, async (req, res) => {
    try {
        ...
        const groupSnapshot = {
          ...
          // -------------------------------------------------
          // AI group harmony (optional, graceful degradation)
          // -------------------------------------------------
          const openaiKey = (process.env.OPENAI_API_KEY || '').trim();
          const openaiBase = (process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1').replace(/\/+\$/, '');
          const openaiModel = (process.env.OPENAI_MODEL || 'gpt-4o-mini').trim();

          let groupHarmony = null;
          let groupAiValid = false;
              ...
              try {
                  ...
                  try {
                      const prevRow = await pool.query(
                          `SELECT iso_week, r2_snapshot_key
                           FROM glp_group_weekly_status_index
                           WHERE cohort_id = $1 AND iso_week < $2
                           ORDER BY iso_week DESC
                           LIMIT 1`,
                          [cohortId, isoWeek]
                      );
                      if (prevRow.rows.length) {
                          prev = await downloadR2Json(prevRow.rows[0].r2_snapshot_key);
                      }
                  } catch (e) {}

                  const prompt = `You are the NAT Lab GLP weekly analyst for group level Good Laboratory Practice compliance.
...

{
  ...
  "strategic_next_action": "string",
  "trend_vs_previous_weeks": "string",
  "focus_for_coming_week": ["string"]
}

Current group snapshot:
${safeJson(groupSnapshot)}
...
${safeJson(prev)}`;
                  ...
                  if (aiResp.ok) {
                      ...
                      let ai = null;
                      if (content) {
                          try { ai = typeof content === 'object' ? content : JSON.parse(content); } catch {}
                      }

                      if (ai && typeof ai === 'object') {
                          groupAiValid = !!(
                              ai.schema_version === 1 &&
                              typeof ai.cohort_id === 'string' &&
                              typeof ai.iso_week === 'string' &&
                              typeof ai.confidence === 'number' &&
                              ai.confidence >= 0 && ai.confidence <= 1 &&
                              Array.isArray(ai.group_wins) && ai.group_wins.length >= 1 &&
                              Array.isArray(ai.systemic_gaps) && ai.systemic_gaps.length >= 1 &&
                              typeof ai.strategic_next_action === 'string' &&
                              typeof ai.trend_vs_previous_weeks === 'string' &&
                              Array.isArray(ai.focus_for_coming_week) && ai.focus_for_coming_week.length >= 1
                          );
                          if (groupAiValid) groupHarmony = ai;
                      }
                  }
              } catch (aiErr) {
                  console.warn('[GLP-GROUP] AI harmony error:', aiErr.message);
              }
          }

          groupSnapshot.has_harmony = groupAiValid;
          groupSnapshot.harmony = groupHarmony;
