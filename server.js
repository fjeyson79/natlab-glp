...
app.post("/api/oligo/upload-pdf", requirePI, upload.single("file"), async (req, res) => {
    try {
        ...
        const { order_no, po_no, items } = parsed;
        ...
        const parse_version = parsed.parse_version;
        ...
        try {
            ...
            await client.query("COMMIT");
            committed = true;
            return res.json({ status: "ok", import_id, item_count: items.length, warnings: parsed.warnings });
...
app.get("/api/oligo/library-certificates/:id/download", requirePI, async (req, res) => {
    try {
        ...
        const url = await getSignedUrl(getR2Client(), new GetObjectCommand({ Bucket: process.env.R2_BUCKET, Key: r.rows[0].r2_object_key }), { expiresIn: 300 });
        res.json({ url });
    } catch (err) {
...
app.get("/api/oligo/syntheses/:id/certificate", requirePI, async (req, res) => {
    try {
        ...
        const { GetObjectCommand } = require("@aws-sdk/client-s3");
        const { getSignedUrl } = require("@aws-sdk/s3-request-presigner");
        const url = await getSignedUrl(getR2Client(), new GetObjectCommand({ Bucket: process.env.R2_BUCKET, Key: key }), { expiresIn: 300 });
        res.json({ url });
    } catch (err) {
